...
{
    return FileNode(m_path, m_size, m_creationDate, m_modificationDate, m_fileType, hash, m_displayName);
}

FileNode FileNode::withDisplayName(std::string displayName) const
{
    return FileNode(m_path, m_size, m_creationDate, m_modificationDate, m_fileType, m_hash, std::move(displayName));
}
...
bool operator==(const FileNode& lhs, const FileNode& rhs) noexcept
{
    return lhs.m_path == rhs.m_path && lhs.m_size == rhs.m_size && lhs.m_creationDate == rhs.m_creationDate
        ...
        && lhs.m_hash == rhs.m_hash && lhs.m_displayName == rhs.m_displayName;
}
